...
| **proxyMode** | 可选      | 代理模式，有效值为： marked 或 matched                      |
 ...
 *
 * 标记代理模式, 只用于浏览器，mock定义的内容将在nodejs环境中运行。
 * 只有当代理模式打开时才有效, 可通过webpack插件(proxyMode=marked) 或者 命令行工具(--proxy marked)打开。
 * 注意: 此特性尚处于实验阶段。
 * @proxy
 */
